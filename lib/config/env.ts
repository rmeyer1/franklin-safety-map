...
const envSchema = z.object({
  ...
  AUDIT_SEVERITY_THRESHOLD: z.coerce.number().int().min(1).max(5).default(5),
  AUDIT_LOW_CONFIDENCE_THRESHOLD: z.coerce.number().min(0).max(1).default(0.65),
  AUDIT_SUPPRESS_CONFIDENCE_THRESHOLD: z.coerce.number().min(0).max(1).default(0.45),
  AUDIT_MIN_GEOCODE_CONFIDENCE: z.coerce.number().min(0).max(1).default(0.7),
